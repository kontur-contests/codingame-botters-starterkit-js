...
export default class StateReader extends BaseStateReader {
    ...
    readInitData() {
        ...
        const staticObjects = Array.from(Array(this.readInt()).keys())
            .map(() => this.readStaticObject())
        const items = Array.from(Array(this.readInt()).keys())
            .map(() => this.readItem())
        printErr('\n')
    ...
    readState(initData) {
        ...
        const units = Array.from(Array(this.readInt()).keys())
            .map(() => this.readUnit())
        printErr('\n')
    ...
    readUnit() {
        ...
        return new Unit(unitId, team, unitType, new Vector(x, y), attackRange, health, maxHealth, shield, attackDamage,
